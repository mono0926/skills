...
`~/Git/script/config/skills.yaml` ファイルを最適化する専属スキルです。不要なスキルを整理し、ユーザーのプロファイルに合った高品質なスキルを補填しつつ、クリーンで安全な環境を維持します。
...
- **共通の目標**: AIのコンテキストウィンドウを最適化された状態に保つこと。ユーザーの主要スタックから外れる無関係なスキルはパフォーマンスを低下させるため極力避ける。
...
現在の `~/Git/script/config/skills.yaml` を読み取ります。インストールされているリポジトリ、ワイルドカードインストール (`*`)、除外されているスキル (`!`) に注目してください。
...
- **絶対に自動で削除しないでください**。
- これらの「スタック外」候補のリストを作成してください。

### 4. 最適化プランの提案

分析・監査結果をユーザーに（日本語で）伝えてください：
...
1. ユーザーが選んだ追加・削除内容に合わせて `~/Git/script/config/skills.yaml` を更新します。
2. 既存のYAMLコメントやフォーマットの構造は維持してください。
3. `~/Git/script/` ディレクトリで `task setup-skills` を実行して変更を反映するよう、ユーザーに伝えます。

## ベストプラクティス

- **提案に対するユーザーの明確な同意がない限り、絶対に `skills.yaml` を変更しないでください**。
- コミュニケーションは役に立つ内容で簡潔に、丁寧かつ親しみやすい日本語で行ってください。
