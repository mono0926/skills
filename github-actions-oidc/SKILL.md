...
#### 共通の重要事項
GitHub ActionsでOIDCトークンを発行するためには、ジョブまたはワークフロー全体に以下の `permissions` 設定が必須です。これを必ずYAMLに含めてください。
```yaml
permissions:
  id-token: write # OIDCトークンの要求に必要
  contents: read  # actions/checkout 等でリポジトリを読み取るために必要
```
