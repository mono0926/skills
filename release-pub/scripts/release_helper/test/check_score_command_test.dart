...
void main() {
  group('CheckScoreCommand', () {
    ...
    test('returns 0 when score is perfect (160/160)', () async {
      ...
      final cmd = CheckScoreCommand(fetcher: (uri) async => mockMetrics);
    ...
    test(
      'returns 0 if min-score is specified and grantedPoints >= minScore',
      () async {
        final mockMetrics = <String, dynamic>{
          'scorecard': <String, dynamic>{
            'packageVersion': '1.0.0',
            'panaReport': <String, dynamic>{
              'report': <String, dynamic>{'sections': <Map<String, dynamic>>[]},
            },
          },
          'score': <String, dynamic>{
            'grantedPoints': 150,
    ...
    test(
      'waits for target version when initial response has older version',
      () async {
        var callCount = 0;
    ...
    test(
      'times out if target version is not analyzed within timeout',
      () async {
        final cmd = CheckScoreCommand(
      ...
      final cmd = CheckScoreCommand(
        fetcher: (uri) async {
          ...
          return <String, dynamic>{
            ...
            'score': <String, dynamic>{'grantedPoints': 160, 'maxPoints': 160},
          };
        },
      );
      runner = CommandRunner<int>('test', 'test')..addCommand(cmd);
